...
struct
  open Config
	...
	(* For part1 only needs to check the last 2 chunks are equal and all remaining chucks are zero *)
	...
	(*  For part2 with max_digits = 10, there are 5 cases here to check:
		[5, 5]
	...
	*)
	...
	let check_bcd bcd = 
		...
		List.map ~f:(fun x -> (split_lsb ~part_width:x ~exact:false bcd |> List.rev |> 
			if String.equal part "part1" then check_repeat_twice else check_repeat_n 
		)) |>
		reduce ~f:(|:)
	;;
  ...
  module I = struct
	...
	let create scope ({clock; clear; uart_in} : _ I.t) : _ O.t
	  =
		let spec = Reg_spec.create ~clock ~clear () in
    let%tydi { valid_out = valid_in; range_begin; range_end; last } = 
