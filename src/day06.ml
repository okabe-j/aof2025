	  ...
	  =
	  	...
	  	if String.equal part "part1" then
				...
				let%hw number 		   = reg_fb spec ~width:result_width ~enable:uart_in.valid
								 								~f:(fun x -> mux2 (~:is_digit) (zero result_width) 
								 								((Util.mul_10 x) +: (uresize ~width:result_width @@ sel_bottom ~width: 4 uart_in.value))) in
		    ...
		    let fifo_data_width = 16 in
				...
				let uart_in_bcd        = uresize ~width:fifo_data_width uart_in.value.:+[0, Some 4] in
				let open Always in
